...
function route(){const id=location.hash.slice(1);if(games[id]){if(!resourcesReady)return;openGame(id,progress[id]?.chapter);}else{active=null;clearTimeout(loadTimer);$('stage').replaceChildren();$('player').hidden=true;$('home').hidden=false;homeProgress()}}
