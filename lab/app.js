...
function route(){const id=location.hash.slice(1);if(games[id]){if(!resourcesReady){$('offline-status').textContent='正在确认离线内容，确认完会自动打开…';return}openGame(id,progress[id]?.chapter);}else{active=null;clearTimeout(loadTimer);$('stage').replaceChildren();$('player').hidden=true;$('home').hidden=false;homeProgress()}}
